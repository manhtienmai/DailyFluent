...
export default function ImportSentenceOrderPage() {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await adminPost<ImportResult>("/crud/exam/import-sentence-order-ai/", { raw_text: rawText, book_id: selectedBook, level, model });
    ...
    <div>
      <Space orientation="vertical" size="large" style={{ width: "100%" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Title level={3} style={{ margin: 0 }}>🔀 AI Import 並べ替え</Title>
          {loading && <Tag color="warning" icon={<Spin size="small" />}>Đang xử lý... {elapsed}s</Tag>}
        </div>
        <Text type="secondary">Paste HTML đề thi sắp xếp câu → Gemini AI extract + giải thích → tự động lưu database</Text>

        <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 20 }}>
          <Card title="Nội dung HTML đề thi sắp xếp câu" size="small">
            <TextArea value={rawText} onChange={(e) => setRawText(e.target.value)} rows={18} style={{ fontFamily: "monospace", fontSize: 13, lineHeight: 1.6 }}
              placeholder={`Paste HTML đề thi sắp xếp câu vào đây...\n\nVí dụ:\n<div class="thithu_ques" data-id="1">\n  <span class="txt1">今度、海外へ転勤することになったので、...</span>\n  ...\n</div>`} />
          ...
          <Space orientation="vertical" size="middle" style={{ width: "100%" }}>
            ...
            <Button type="primary" block icon={<RobotOutlined />} loading={loading} disabled={textLines === 0} onClick={handleImport} size="large">
              {loading ? `Đang xử lý AI... (${elapsed}s)` : "🔀 AI Extract & Import 並べ替え"}
            </Button>
            {loading && <Text type="secondary" style={{ textAlign: "center", display: "block", fontSize: 12 }}>Thời gian xử lý khoảng 30-60s cho 12 câu.</Text>}
            ...
            <Card size="small" title="📋 Hướng dẫn">
              <Text style={{ fontSize: 12 }}>1. Copy HTML đề thi 並べ替え<br />2. Paste vào ô bên trái<br />3. Chọn trình độ + sách<br />4. Click <strong>AI Extract & Import</strong><br />5. AI tự trích xuất câu hỏi, thứ tự đúng và giải thích</Text>
            </Card>
          </Space>
        </div>
      </Space>
